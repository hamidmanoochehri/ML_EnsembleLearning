# -*- coding: utf-8 -*-
"""
Created on Fri Oct 21 21:10:44 2022
...
# def bagging_test(T,df,h_list,max_depth):
#     correct=0
#     for row in range (0,df.shape[0]):
#          df_rows = df.iloc[row , : ]
#          H=0
...
#              H= H + h
#          pr_label=int(np.sign(H))
#          if pr_label==0:
#              pr_label=+1
#          if pr_label==df_rows.label:
#              correct+=1
#     error=(df.shape[0]-correct)/df.shape[0]
#     return error
...
def bias_variance(n,T,df,whole_bagged_trees,single_trees,max_depth):
    ...
    print ('Bagged trees : bias_mean_bag {} , variance_mean_bag {}'.format(bias_mean_bag,variance_mean_bag))
...
whole_bagged_trees=[]
for i in range (0,n):
    h_list = bagging_train(T, df_train_m, max_depth, Entropy_method,att_barnches)
